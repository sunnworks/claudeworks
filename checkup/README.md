...
| `npm run test` | 규칙엔진·점수계산·검증·문항데이터·모름처리·준비사항 단위테스트 65개 |
| `npm run test:e2e` | 4개 시나리오 완주, 인수기준, 접근성·수어·레이아웃 E2E 44개 (데스크톱 1280 / 모바일 360) |
...
* **영상 영역은 고정 크기**입니다. 문항이 바뀌어도 화면이 흔들리지 않습니다(E2E 검사).
  기본은 4:3 확대이고 `화면 작게`로 16:9 원본 비율을 볼 수 있습니다.
