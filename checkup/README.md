...
* **자막**: 영상 바로 아래 띠로 붙입니다. 영상 위에 겹치면 수어 동작을 가립니다.
  **문항 영상은 자막을 붙이지 않습니다** — 같은 문장이 바로 아래 질문으로 또 나오기 때문입니다.
  선택지·안내 영상은 무엇을 보여 주는지 알 수 없으므로 반드시 붙입니다. `설정 → 자막 끄기`로 끌 수 있습니다.
* **영상 폭은 아래 작성 칸과 같습니다.** 문항이 바뀌어도 크기가 흔들리지 않습니다(E2E 검사).

  | 화면 | 기본 | `화면 크게` |
  | --- | --- | --- |
  | 데스크톱 | 16:9 · 높이 최대 46vh | **1:1 정사각형** · 최대 74vh |
  | 모바일(≤720px) | 4:3 · 높이 최대 30vh | 1:1 정사각형 · 최대 62vh |
