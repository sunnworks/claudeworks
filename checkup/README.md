...
| `npm run test:e2e` | 4개 시나리오 완주, 인수기준, 접근성 E2E 34개 (데스크톱 1280 / 모바일 360) |
...
이 데모의 목적은 **농인이 수어영상만 보고 혼자서 문진표를 끝까지 작성**하는 것입니다.
따라서 문진 문항뿐 아니라 **모든 화면의 모든 질문·선택지·안내문에 수어 버튼**이 있습니다.

* 수어영상 패널은 **모든 화면(S01~S10)에 같은 자리**에 있습니다. 넓은 화면은 왼쪽 고정, 모바일은 맨 위.
* 문구 옆 **▶ 수어** 버튼을 누르면 그 문구가 자막으로 바뀌며 영상이 재생됩니다.
  문항 본문 · 쉬운 설명 · 도움말 · 각 선택지 · 표의 각 행과 열 · 화면 이동 버튼까지 모두 제공합니다.
* **자동재생**: 화면이나 문항이 바뀌면 자동으로 재생합니다(기본 켜짐).
  영상에 소리가 없어 브라우저 자동재생 정책에 걸리지 않습니다. `영상 설정 → 자동재생 끄기`로 끌 수 있습니다.
  설계서 13의 "영상 자동재생 금지"는 소리 있는 매체를 전제한 조항이므로, 무음 영상 + 항상 보이는 정지 버튼 +
  끄기 설정으로 대체했습니다.
* **영상 영역은 고정 크기**입니다. 문항이 바뀌어도 화면이 흔들리지 않습니다.
  기본은 `크게 보기`(4:3, 원본의 좌우 검은 여백만 잘림)이고, `작게 보기`로 16:9 원본 비율을 볼 수 있습니다.
  수어는 손이 움직이는 공간이 의미를 가지므로 **동작이 잘리는 확대는 하지 않습니다.**
* 제어: 재생·정지 / 다시보기 / 1·0.75·0.5배속 / 전체화면, 접은 메뉴에 크기·자동재생·다른 샘플영상.
...
| 추가 | 영상 크기 고정 · 자동재생 전환 | `accessibility.spec.ts` | 자동 |
