...
for (const scenario of SCENARIOS) {
  test(`AC-01·AC-02 ${scenario.name} 시나리오를 끝까지 완료한다`, async ({ page }) => {
    ...
    await page.getByRole('button', { name: '문진 시작' }).click();
    ...
    await expect(page.getByText('제출되지 않았습니다')).toBeVisible();
  });
}
...
test('AC-06 PHQ-9 9번에 1점 이상 답하면 안전안내가 먼저 표시된다', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: '시작하기' }).click();
  await page.getByRole('button', { name: /C 시나리오/ }).click();
  await page.getByRole('button', { name: '다음' }).click();
  await page.getByRole('button', { name: '문진 시작' }).click();
...
test('AC-09·AC-10 답변이 저장소·URL·콘솔에 남지 않는다', async ({ page }) => {
  ...
  await page.goto('/');
  await page.getByRole('button', { name: '시작하기' }).click();
  await page.getByRole('button', { name: /A 시나리오/ }).click();
  await page.getByRole('button', { name: '다음' }).click();
  await page.getByRole('button', { name: '문진 시작' }).click();
...
test('AC-11 360px 폭에서 가로 스크롤이 생기지 않는다', async ({ page }) => {
  ...
  await page.goto('/');
  await page.getByRole('button', { name: '시작하기' }).click();
  await page.getByRole('button', { name: /B 시나리오/ }).click();
  await page.getByRole('button', { name: '다음' }).click();
  await page.getByRole('button', { name: '문진 시작' }).click();
